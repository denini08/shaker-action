...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield exec.exec('sh', ['-c', 'echo', adbRunTestsComand, '>>', `${path_src}tests.sh`]);
            yield exec.exec('sh', ['-c', 'ls', path_src]);
            yield exec.exec('sh', ['-c', 'cat', `${path_src}tests.sh`]);
            console.log("AQUI");
            // await exec.exec('sh', ['-c', 'find / -name tsts.py -print -quit']);
            //console.log("!!! RUNNING TEST by js")
            yield exec.exec('python3', [path_src + "tsts.py"]);
