...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            process.chdir("../");
            console.log("AQUI");
            yield exec.exec('sh', ['-c', 'find / -name tsts.py -print -quit']);
            console.log("!!! RUNNING TEST by js");
            yield exec.exec('python3', ['tsts.py']);
            console.log("!!! RUNNING SHAKER by js");
            yield exec.exec('python3', ['exec.py', '3', 'name']);
            ...
            yield exec.exec('sh', ['-c', 'echo', adbRunTestsComand, '>>', 'tests.sh']);
            yield exec.exec('sh', ['-c', 'cat', 'exec.py']);
            yield exec.exec('sh', ['-c', 'cat', 'tests.sh']);
